...
impl Builtin {
    pub fn load() -> &'static Self {
        ...
        INSTANCE.get_or_init(|| {
            let template_labels =
                yaml::from_slice::<FieldSpecs>(TEMPLATE_LABELS).expect("template labels");

            let other_labels = yaml::from_slice::<FieldSpecs>(OTHER_LABELS).expect("other labels");
            ...
            Builtin {
                subjects: yaml::from_slice::<FieldSpecs>(SUBJECTS)
                    .expect("subjects (other labels)"),
                common_annotations: yaml::from_slice::<FieldSpecs>(COMMON_ANNOTATIONS)
                    .expect("common annotations"),
                common_labels,
                images: yaml::from_slice::<FieldSpecs>(IMAGES).expect("images"),
                template_labels,
